...
|---|---|---|
| Tier A | ліквідність (ціна >$10, капіталізація >$10 млрд, $ обсяг >$10 млн/день) | 365 / 491 пройшли |
| Tier B | фундаментал (прибутковість, revenue/EPS YoY, дилюція, L/A) | 35 / 411 пройшли |
| Tier C | valuation (P/E, P/S, PEG) | 16 / 35 пройшли |
| Composite score | ранжування пройшли-Tier-C за формулою (не фільтр) | готово, чекає фінального живого прогону користувача |

Тести: 113/113 (`docker compose exec app pytest -q`) — 67 data-ingestion/reporting
+ 46 analysis/screening.
...
- `docs/watchlist.md` пріоритет 2 — загальні активи для news/, ще не
  визначені разом з користувачем
- Крипто-монети для watchlist — BTC+ETH взято як припущення, не
  підтверджено користувачем
- Нафта — WTI, Brent, чи обидва — не уточнено
